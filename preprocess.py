...
def removeRandomData(dataset, percentage): #remove based on removing percentage
    for row in dataset:
        removed = random.sample(range(1, len(list(row))-1), int(percentage*len(list(row))))
        #print(removed)
        for i in removed:
            if row[i]!='class_label':
                del row[i]
    return dataset
...
def removeDataTrapezoidal(original_dataset): #trapezoidal
    ...
    for i in range(0, len(dataset)):
        multiplier=int(i/int(rows/10))+1
