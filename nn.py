...
def experiment(X_list, y_list):
    ...
    model = DynamicNet(9, H, D_out)
    print(model.parameters)
    criterion = torch.nn.L1Loss(size_average=False)
    #criterion = torch.nn.SoftMarginLoss(size_average=False)
    ...
    for t in range(len(y_list)):
        ...
        #print(np.count_nonzero(X_list[t]))
        #print(len(X_list[t]))
        #print(len(truncTrapez(X_list[t], t, len(X_list))))
        #print()
        ...
        if adaptive_lr_switch == 1:
            if feature_list[t]>len_prev:
                #rate = learning_rate*(np.sqrt(len(x)-len_prev))
                rate = learning_rate * (entropy_list[t-1])
                
                if prune_switch == 1:
                     #model.projectWeights(int(np.sqrt(len(x)))) # node based pruning
        ...
        if model.input_linear.weight.data.shape[1]<x.data.shape[0]:
            print(model.input_linear.weight.data.shape[1])
...
class DynamicNet(torch.nn.Module):
    def __init__(self, D_in, H, D_out):
        ...
        #self.middle_linear1 = torch.nn.Linear(H, int(H/4))
        #self.middle_linear2 = torch.nn.Linear(int(H/4), int(H/16))
        #self.middle_linear3 = torch.nn.Linear(int(H/4), int(H/8))
        self.output_linear = torch.nn.Linear(H, D_out, bias=False)
    ...
    def transferWeights(self, x, layer=None):
        ...
        print("transfer weights called")
        if layer==None:
            layer = self.input_linear
        current_input_size = layer.weight.size()[1] #input dimension?
        new_input_size = x.size()[0]   
        a = torch.zeros(layer.weight.size()[0], new_input_size-current_input_size)
        b = torch.cat((self.input_linear.weight.data, a), 1)
        #print("previous weights")
        #print(self.input_linear.weight.data)
        #print("appendix weights")
        #print(a)
        self.input_linear = torch.nn.Linear(b.size()[1], b.size()[0])
        self.input_linear.weight.data = b
        #print("appended weights")
        #print(self.input_linear.weight.data)
        #print()
    ...
    def pruneEdges(self, n):
        wts = self.input_linear.weight.data.numpy()
        wts2 = self.output_linear.weight.data.numpy()
        norm2 = np.linalg.norm(np.absolute(wts2),ord=1)
        norm = np.linalg.norm(np.absolute(wts),ord=1)
        projected_wts = np.multiply(lamb/norm, wts)
        projected_wts2 = np.multiply(lamb/norm2, wts2)
        ...
        self.output_linear.weight.data = torch.from_numpy(projected_wts2).float()              
